  ...
  def normalize(self):
    ...
    for resultsFile in self.resultsFiles:
      ...
      with open(resultsFile) as f:
        ...
        recall_new = (results["recall_numerator"].iloc[-1])*1.0/(results["rar_count"].iloc[-1])
        precision_new = (results["precision_numerator"].iloc[-1])*1.0/(results["par_count"].iloc[-1])
        # Add to results dict:
      ...
      print ("Final score for \'%s\' detector on \'%s\' profile = %.2f, recall_new =\'%s\', precision_new=\'%s\'"
             % (detector, profile, score, recall_new, precision_new))
